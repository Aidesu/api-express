...
const Satellite = mongoose.model("Satellite", satSchema);
...
export async function deleteSatellites(id) {
    // satellites.splice(id, 1);
    await Satellite.findByIdAndDelete(id);
}

export async function updateSatellites(satObj, id) {
    // const indexOfSat = satellites.findIndex((s) => s.id == satObj.id);
    // satellites[indexOfSat] = { ...satellites[indexOfSat], ...satObj };
    await Satellite.updateOne({ id: id }, { satObj });
